...
const pool2f = mariadb.createPool({
  host: 'localhost',
  port: 3306,
  user: 'root', 
  password: 'Aa111111', 
  database: 'db_scadasyntec',
  connectionLimit: 10 // 连接池大小，可根据需要调整
});
const pool3f = mariadb.createPool({
  host: 'localhost',
  port: 3306,
  user: 'root', 
  password: 'Aa111111', 
...
//get接口示例(http://localhost:3000/api/GetLed2F/202512141443)
app.get('/api/GetLed2F/:ScadaNO', async (req, res) => {
  // GET 路由参数从 req.params 获取
  const { ScadaNO } = req.params as { ScadaNO?: string };  
...
//get接口示例(http://localhost:3000/api/GetLed3F/202512141443)
app.get('/api/GetLed3F/:ScadaNO', async (req, res) => {
  // GET 路由参数从 req.params 获取
  const { ScadaNO } = req.params as { ScadaNO?: string };  
...
app.post('/api/GetSum2F', async (req, res) => {
  // 1. 从请求体（req.body）中获取参数
  const { DayT1, DayT2, LastT1, LastT2, ThisT1, ThisT2 } = req.body as {
    DayT1?: string;
    DayT2?: string;
...
app.post('/api/GetSum3F', async (req, res) => {
  // 1. 从请求体（req.body）中获取参数
  const { DayT1, DayT2, LastT1, LastT2, ThisT1, ThisT2 } = req.body as {
    DayT1?: string;
    DayT2?: string;
